if game.CoreGui:FindFirstChild("Crimware1") then
    game.CoreGui.Crimware1:Destroy()
end
...
local GenerateGUID = Crimware_Main.HttpService:GenerateGUID(false) 
...
function Crimware_Main:PlaySound(SoundID)
...
local Crimware = Instance.new("ScreenGui")
Crimware.Name = "Crimware1"
Crimware.Parent = game.CoreGui
Crimware.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

-- -- this function converts a string to base64
-- function Crimware_Main:encode(data)
...
-- function Crimware_Main:decode(data)
--     local b = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'
...
function Crimware_Main:encode(data)
    ...
    encodedData = "-- CrimWare by 21xDecay \n\nreturn function() "..insertRandomWords2(encodedData)..' end) \n\n\n'

    return encodedData
end

function Crimware_Main:decode(data)
    data = data:gsub('^-- Crimware by 21xDecay %s+return%s+function%(%) (.*) end%)%s*\n*$', '%1')
...
function Crimware_Main:GetDistance(player_pos, Endpoint)
...
function Crimware_Main:GetPlayerImage(ID)
    local width = 420
    local height = 420
    local format = "png"
...
function Crimware_Main:SetCFG(name)
    Crimware_Main.Targeted_Config = name
end

function Crimware_Main:LoadSettings(Folder, CFGName)

    local Encoded = readfile(Folder .. "/settings.txt")
    local Decoded = Crimware_Main:decode(Encoded)

    writefile(Folder .. "/settings.txt", tostring(Crimware_Main.HttpService:JSONEncode(Decoded)))

    Crimware_Main.Settings = Crimware_Main.HttpService:JSONDecode(readfile(Folder .. "/settings.txt"))
end

function Crimware_Main:AutoJoinDiscord(DiscordCode)
    ...
    if req then
        req({
            ...
            Body = Crimware_Main.HttpService:JSONEncode({
                cmd = 'INVITE_BROWSER',
                nonce = Crimware_Main.HttpService:GenerateGUID(false),
...
function ChangeTypeText(object)
    ...
    {TextColor3 = Crimware_Main.Theme.Custom.Text}
):Play()
end
...
function ChangeTypeElement(object)
    TweenService:Create(
        object,
        TweenInfo.new(.3, Enum.EasingStyle.Quad),
        {BackgroundColor3 = Crimware_Main.Theme.Custom.Element}
    ):Play()
end

function Crimware_Main:Window(config)
    ...
    function Crimware_Main:GetConfigNames()
        local ReturnTable = {}
    ...
    if not isfile(Folder .. "/settings.txt") then
        local content = {}
        for i,v in pairs(Crimware_Main.Settings) do
            content[i] = v
        end
        writefile(Folder .. "/settings.txt", tostring(Crimware_Main.HttpService:JSONEncode(content)))
    end
    Crimware_Main.Settings = Crimware_Main.HttpService:JSONDecode(readfile(Folder .. "/settings.txt"))
    ...
    function SaveSettings(bool)
        local rd = Crimware_Main.HttpService:JSONDecode(readfile(Folder.."/settings.txt"))
        state = bool
        if state then
            return rd
        end
        ...
        -- writefile(Folder .. "/settings.txt", tostring(Crimware_Main.HttpService:JSONEncode(Crimware_Main:encode(content))))
        writefile(Folder .. "/settings.txt", tostring(Crimware_Main.HttpService:JSONEncode(content)))
    end
    ...
    function SaveSettingsCFG(cfg)
        local content = {}
        for i, v in pairs(Crimware_Main.SettingsFlags) do
            content[i] = v.Value
        end
    
        local Encoded = Crimware_Main.HttpService:JSONEncode(content) -- Use HttpService
    ...
    function LoadSettingsCFG(cfg)
        ...
        local JSONData = Crimware_Main.HttpService:JSONDecode(Encoded) -- Use HttpService
    
        for a, b in pairs(JSONData) do
            if Crimware_Main.SettingsFlags[a] then
                spawn(function()
                    Crimware_Main.SettingsFlags[a]:Set(b)
                end)
            else
                warn("Error ", a, b)
            end
    ...
    function EditSettingsCFG(cfg, Name, newvalue)
        ...
        if Crimware_Main.SettingsFlags[Name] then
            spawn(function()
                Crimware_Main.SettingsFlags[Name]:Set(newvalue)
            end)
        end
    end
    ...
    function SystemT(title, callback)
        ...
        SetupSystemToggle.MouseButton1Click:Connect(function()
            Crimware_Main:PlaySound(Crimware_Main.Lib_Sounds.ClickSound)
            SToggled = not SToggled
            SystemTogglefunc:Set(SToggled)
        end)

        Crimware_Main.SettingsFlags[title] = SystemTogglefunc
    ...
    spawn(function()
        while wait() do
            if RememberKey == false and HasBeenToggled == false then
                pcall(function()
                    EditSettingsCFG("KeyCrimware", "Key Holder", "")
                end)
            end
        end
    end)
    ...
    function SystemK(title, callback)
        ...
        Crimware_Main.SettingsFlags[title] = KeyBoxfunc
        return KeyBoxfunc
    end
    ...
    function Crimware_Main:VisMain(v)
        MainFrame.Visible = v
    end
    ...
    PlayerImage.Image = Crimware_Main:GetPlayerImage(Player.UserId)
    ...
    function Crimware_Main:Notify(cfg)
    ...
    --[[
        ...
        for i,v in pairs(ListScripts) do
            ...
            ShareScript.MouseEnter:Connect(function()
                Crimware_Main:PlaySound(Crimware_Main.Lib_Sounds.HoverSound)
            end)
        end
        ...
        --             Method = "GET"
        --         })
        --         local data = Crimware_Main.HttpService:JSONDecode(data.Body)
        ...
        --                 ChatSocketFrame.Parent = ChatFrameFrame --game:GetService("CoreGui").Crimware1.MainFrame.ChatFrame.ChatFrameFrame
        ...
        ChatBoxText.FocusLost:Connect(function(ep)
            if ep then
                local Data = Crimware_Main.HttpService:JSONEncode({
        ...
        spawn(function()task.wait(.5)Crimware_Main:AutoJoinDiscord("qq6WgyMwkw")end)
    ...
    function TabsSec:TSection(title)
        ...
        -- function Crimware_Main:LoadCfg(cfg)
        --     local Encoded = readfile(Folder1 .. "/configs/" .. cfg .. ".txt")
        --     local Decoded = Crimware_Main:decode(Encoded)
            
        --     local Encode = Crimware_Main.HttpService:JSONEncode(Decoded)
        ...
        --         print(a,b)
        --     if Crimware_Main.Flags[a] then
        --         spawn(function()
        --           Crimware_Main.Flags[a]:Set(b)
        --        end)
        --     else
        --         warn("Error ", a,b)
        --     end
        ...
        -- function Crimware_Main:SaveCfg(cfg)
        --     local content = {}
        --     for i,v in pairs(Crimware_Main.Flags) do
        --         content[i] = v.Value
        --     end
        --     writefile(Folder1.."/configs/"..cfg..".txt", Crimware_Main:encode(tostring(Crimware_Main.HttpService:JSONEncode(content)))) -- FolderName.."/configs/"..name..".cfg"
        -- end
      
        -- function Crimware_Main:CreateCfg(cfg)
        --     local content = {}
        --     for i,v in pairs(Crimware_Main.Flags) do
        --         content[i] = v.Value
        --     end
        --     writefile(Folder1.."/configs/"..cfg..".txt", Crimware_Main:encode(Crimware_Main.HttpService:JSONEncode(content))) -- FolderName.."/configs/"..name..".cfg"
        --     -- writefile("Crimware/configs/Mana64.txt", Crimware_Main:encode(tostring(content)))
        -- end

        function Crimware_Main:LoadCfg(cfg)
            local Encoded = readfile(Folder1 .. "/configs/" .. cfg .. ".txt")

            local JSONData = Crimware_Main.HttpService:JSONDecode(Crimware_Main:decode(Encoded))
        ...
        function Crimware_Main:SaveCfg(cfg)
            ...
            local Encoded = Crimware_Main:encode(Crimware_Main.HttpService:JSONEncode(content))
            
            writefile(Folder1 .. "/configs/" .. cfg .. ".txt", Encoded)
        end

        function Crimware_Main:Edit_LastLoad(cfg)
            writefile(Folder1.."/LastLoaded.txt", Crimware_Main.HttpService:JSONEncode({["CFG"] = tostring(cfg)}))
        end

        function Crimware_Main:LastConfigSaved()
            if isfile(Folder1.."/LastLoaded.txt") then
                return Crimware_Main.HttpService:JSONDecode(readfile(Folder1.."/LastLoaded.txt")).CFG
            else
                Crimware_Main:Notify({Title = "Crimware",
        ...
        function Crimware_Main:CreateCfg(cfg)
            ...
            local Encoded = Crimware_Main.HttpService:JSONEncode(content) -- Convert to JSON string
            
            writefile(Folder1 .. "/configs/" .. cfg .. ".txt", Encoded)
        end
        
        SaveCFGB.MouseButton1Click:Connect(function()
            if Crimware_Main.Targeted_Config == "" then
                Crimware_Main:Notify({Title = "Crimware",
                ...
                Crimware_Main:Notify({Title = "Crimware",
                    Text = "Saved to: "..tostring(Crimware_Main.Targeted_Config),
                    Time = 2,
                    AutoClose = true
                })
                Crimware_Main:SaveCfg(tostring(Crimware_Main.Targeted_Config))
                Edit_LastLoad(tostring(Crimware_Main.Targeted_Config))
            end
        end)
        ...
        function Tabs:Tab(title)
            ...
            function Sections:Section(title)
                    ...
                    while task.wait() do
                        pcall(function()
                            TweenService:Create(
                                Section,
                                TweenInfo.new(.3, Enum.EasingStyle.Quad),
                                {BackgroundColor3 = Crimware_Main.Theme.Custom.Section}
                            ):Play()
                        end)
                    end
                end)
                    ...
                    spawn(function()
                        while task.wait() do
                            pcall(function()
                                ...
                                ButtonStroke.Color = Crimware_Main.Theme.Custom.Element
                            end)
                        end
                    end)
                ...
                function Elements:Toggle(title, callback)
                    ...
                    Crimware_Main.Flags[title] = Togglefunc
                    return Togglefunc
                end
                ...
                function Elements:Dropdown(title, list, callback)
                    ...
                    spawn(function()
                        while task.wait() do
                            pcall(function()
                                ...
                                Arrow.ImageColor3 = Crimware_Main.Theme.Custom.Element
                            end)
                        end
                    end)
                    ...
                    Crimware_Main.Flags[title] = Dropfunc
                    return Dropfunc
                end
                ...
                function Elements:Slider(title, min, max, start, callback)
                    ...
                    Crimware_Main.Flags[title] = Sliderfunc
                    return Sliderfunc
                end
                ...
                function Elements:Colorpicker(title, preset, callback)
                        ...
                        while task.wait() do
                            pcall(function()
                                TweenService:Create(
                                    ColorPFrameGlow,
                                    TweenInfo.new(.4, Enum.EasingStyle.Quad),
                                    {ImageColor3 = Crimware_Main.Theme.Custom.Glow}
                                ):Play()
                            end)
                        end
                    end)
                        ...
                        while task.wait() do
                            pcall(function()
                                TweenService:Create(
                                    ColorpreviewStroke,
                                    TweenInfo.new(.3, Enum.EasingStyle.Quad),
                                    {Color = Crimware_Main.Theme.Custom.Element}
                                ):Play()
                           ...
                           if input.UserInputType == Enum.UserInputType.MouseButton1 then
                              ...
                              ColorInput =
                                 RunService.RenderStepped:Connect(
                                    function()
                                    local ColorX =
                                       (math.clamp(Crimware_Main.Mouse.X - ChoseColor.AbsolutePosition.X, 0, ChoseColor.AbsoluteSize.X) /
                                       ChoseColor.AbsoluteSize.X)
                                    local ColorY =
                                       (math.clamp(Crimware_Main.Mouse.Y - ChoseColor.AbsolutePosition.Y, 0, ChoseColor.AbsoluteSize.Y) /
                                       ChoseColor.AbsoluteSize.Y)
                           ...
                           if input.UserInputType == Enum.UserInputType.MouseButton1 then
                              ...
                              HueInput =
                                 RunService.RenderStepped:Connect(
                                    function()
                                    local HueY =
                                       (math.clamp(Crimware_Main.Mouse.Y - Hue.AbsolutePosition.Y, 0, Hue.AbsoluteSize.Y) /
                                          Hue.AbsoluteSize.Y)
                    ...
                    Crimware_Main.Flags[title] = Colorpickerfunc
                ...
                function Elements:Bind(title, callback, external)
                    ...
                    AddBind.MouseButton1Click:Connect(function()
                        ...
                        TweenService:Create(
                            BindSettingImage,
                            TweenInfo.new(.3, Enum.EasingStyle.Quad),
                            {ImageColor3 = Crimware_Main.Theme.Custom.Element}
                        ):Play()
    ...
    spawn(function()
        ...
        Sec2:Button("Create Config", function()
            Crimware_Main:CreateCfg(tostring(Config_Name))
            Crimware_Main:Notify({
                Title = "Crimware",
        ...
        local Configs_Drop = Sec1:Dropdown("Select Config", Crimware_Main:GetConfigNames(), function(t)
            Selected_Config = t
            Crimware_Main:Notify({Title = "Crimware",
                Text = "Targeted CFG: "..tostring(Selected_Config)
            })
            Crimware_Main:SetCFG(tostring(Selected_Config))
        end)

        Sec1:Button("Refresh Configs", function()
            Configs_Drop:Refresh(Crimware_Main:GetConfigNames())
        end)
        Sec1:Line()
        Sec1:Button("Load Selected Config", function()
            ...
            Crimware_Main:LoadCfg(tostring(Selected_Config))
            Crimware_Main:Edit_LastLoad(tostring(Selected_Config))
        end)
        Sec3:Colorpicker("Background", Crimware_Main.Theme.Custom.Background, function(t)
            Crimware_Main.Theme.Custom.Background = t
        end)
        Sec3:Colorpicker("Section", Crimware_Main.Theme.Custom.Section, function(t)
            Crimware_Main.Theme.Custom.Section = t
        end)
        Sec3:Colorpicker("Element", Crimware_Main.Theme.Custom.Element, function(t)
            Crimware_Main.Theme.Custom.Element = t
        end)
        Sec3:Colorpicker("Text", Crimware_Main.Theme.Custom.Text, function(t)
            Crimware_Main.Theme.Custom.Text = t
        end)
        Sec3:Colorpicker("Glow", Crimware_Main.Theme.Custom.Glow, function(t)
            Crimware_Main.Theme.Custom.Glow = t
        end)
        ...
        if not isfile(Folder..'/On_Launch.json') then
            writefile(Folder..'/On_Launch.json', Crimware_Main.HttpService:JSONEncode({
                ["On_Launch"] = false,
            }))
        end
        ...
        local Get_Load = Crimware_Main.HttpService:JSONDecode(readfile(Folder..'/On_Launch.json')).On_Launch

        Get_ALC:Set(Get_Load)

        game.CoreGui:WaitForChild("Crimware1").MainFrame.Visible = Get_Load
    ...
    spawn(function()
        while task.wait() do
            pcall(function()
                --// Background \\--
                KeyFrame.BackgroundColor3 = Crimware_Main.Theme.Custom.Background
                MainFrame.BackgroundColor3 = Crimware_Main.Theme.Custom.Background
                LeftFrame.BackgroundColor3 = Crimware_Main.Theme.Custom.Background
                ...
                --// Element \\
                -- Crimware_Main.Theme.Custom.Section
                -- Crimware_Main.Theme.Custom.Element
                -- Crimware_Main.Theme.Custom.Text
            end)
        end
    end)
    return TabsSec
